...
class DataTable2 extends DataTable {
  ...
  /// Index of the currently selected row (0-based), null if no row is selected
  final int? selectedRowIndex;

  /// Callback triggered when a row is selected by index
  final ValueChanged<int>? onRowSelected;

  /// The height of each data row used for calculating scroll position
  /// If not provided, defaults to [dataRowHeight] or standard height
  double get effectiveRowHeight => dataRowHeight ?? kMinInteractiveDimension;
  ...
  /// - [shouldSelect]: If true, triggers [onRowSelected] callback
  void scrollToRow(
    int rowIndex, {
    ...
    if (scrollController == null || rowIndex < 0 || rowIndex >= rows.length) {
      return;
    }

    // Calculate position accounting for header row
    final headingRowHeight =
        (kMinInteractiveDimension) + (dividerThickness ?? 1.0);
    final position = (rowIndex * effectiveRowHeight) +
        (fixedTopRows > 0 ? 0 : headingRowHeight);
    ...
    // Trigger selection callback if requested
    if (shouldSelect && onRowSelected != null) {
      onRowSelected!(rowIndex);
    }
  }

  /// Highlights the selected row by updating its visual state
  /// This should be called when building data rows to apply highlight styling
  bool isRowSelected(int rowIndex) {
    return selectedRowIndex != null && selectedRowIndex == rowIndex;
  }
